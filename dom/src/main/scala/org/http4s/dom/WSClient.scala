...
object WSClient {
  ...
  private val DefaultConnectionTimeout = 30.seconds
  private val DefaultCloseTimeout = 10.seconds
  private val DefaultReceiveTimeout = 1.minute
      ...
      for {
        ...
        ws <- Resource.makeCase {
          F.race(
            ...
            F.sleep(DefaultConnectionTimeout) *> F.raiseError[WebSocket](
              new WSException("WebSocket connection timed out after " + DefaultConnectionTimeout.toString))
          ).flatMap(F.pure(_))
        } {
          case (ws, exitCase) =>
            ...
            F.race(
              ...
              F.sleep(DefaultCloseTimeout) *> F.raiseError[CloseEvent](
                new WSException("WebSocket close operation timed out after " + DefaultCloseTimeout.toString))
        ...
        def receive: F[Option[WSDataFrame]] =
          F.race(
            drained
              ...
              .map(_.merge),
            F.sleep(DefaultReceiveTimeout) *> F.raiseError[Option[WSDataFrame]](
              new WSException("WebSocket receive operation timed out after " + DefaultReceiveTimeout.toString))
          )
